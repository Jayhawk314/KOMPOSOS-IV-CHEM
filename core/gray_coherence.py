...
"""
Gray-Category 3-Cell Coherence Layer
=====================================
...
---------------------
    coherence gap type          →  vulnerability class
    ─────────────────────────────────────────────────
    interchange_failure         →  type_confusion
    composition_boundary        →  buffer_overflow
    lifetime_violation          →  use_after_free
    privilege_non_commute       →  privilege_escalation
    functor_escape              →  sandbox_escape
    modification_missing        →  race_condition
    gray_tensor_failure         →  memory_corruption
    sieve_collapse              →  authentication_bypass
...
Import in cosmos.py::InfinityCosmos:
    ...
    from komposos.core.gray_coherence import (
        GrayCategoryLayer, Modification, VulnerabilityCandidate,
        SoftwareCategoryBuilder, MythosRace,
    )
...
The MythosRace scanner is the pre-emptive shield — run it
...
class CoherenceGapType(Enum):
    ...
    Each maps directly to a vulnerability class.
    """
...
VULN_CLASS: Dict[CoherenceGapType, str] = {
    CoherenceGapType.INTERCHANGE_FAILURE:   "type_confusion",
    CoherenceGapType.COMPOSITION_BOUNDARY:  "buffer_overflow",
    CoherenceGapType.LIFETIME_VIOLATION:    "use_after_free",
    CoherenceGapType.PRIVILEGE_NON_COMMUTE: "privilege_escalation",
    CoherenceGapType.FUNCTOR_ESCAPE:        "sandbox_escape",
    CoherenceGapType.MODIFICATION_MISSING:  "race_condition",
    CoherenceGapType.GRAY_TENSOR_FAILURE:   "memory_corruption",
    CoherenceGapType.SIEVE_COLLAPSE:        "authentication_bypass",
...
class SoftwareCategoryBuilder:
    ...
    def __init__(self):
        self.objects: Dict[str, SoftwareObject] = {}
        self.morphisms: List[SoftwareMorphism] = []
    ...
    def from_source_file(self, path: str) -> "SoftwareCategoryBuilder":
    ...
    def from_source_dir(self, directory: str,
                        glob: str = "**/*.py") -> "SoftwareCategoryBuilder":
    ...
    def from_cfg(
        ...
        """
        for n in nodes:
            self.objects[n["id"]] = SoftwareObject(
                name=n["id"],
                kind=n.get("kind", "node"),
                privilege=n.get("privilege", 0),
            )
        for e in edges:
            self.morphisms.append(SoftwareMorphism(
                source=e["src"],
                target=e["dst"],
    ...
    def from_api_spec(
        ...
        """
        for ep in endpoints:
            self.objects[ep] = SoftwareObject(name=ep, kind="endpoint")
    ...
    def enumerate_2cell_pairs(self) -> List[Tuple[TwoCellProxy, TwoCellProxy]]:
        ...
        by_source: Dict[str, List[SoftwareMorphism]] = {}
                ...
                for j in range(i + 1, len(morphs)):
                    ...
                    alpha = TwoCellProxy(
                        ...
                        privilege_level=self.objects.get(m1.target,
                            SoftwareObject(m1.target, "unknown")).privilege,
                    ...
                    beta = TwoCellProxy(
                        ...
                        privilege_level=self.objects.get(m2.target,
                            SoftwareObject(m2.target, "unknown")).privilege,
    ...
    def _walk_ast(self, tree: ast.AST, module_name: str):
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                self.objects[fn_name] = SoftwareObject(
    ...
    def _privilege_delta(self, src: str, dst: str) -> int:
        src_priv = self.objects.get(src, SoftwareObject(src, "x")).privilege
        dst_priv = self.objects.get(dst, SoftwareObject(dst, "x")).privilege
...
class GrayCategoryLayer:
    ...
    def enumerate_modification_candidates(
        self, builder: SoftwareCategoryBuilder,
...
class CoherenceVulnerabilityMapper:
    """
    Turns Modification objects into VulnerabilityCandidate reports.

    The Modification IS the proof — no separate certificate needed.
    """

    def classify(self, mod: Modification, location: str = "") -> VulnerabilityCandidate:
        vuln_class = VULN_CLASS[mod.gap_type]
...
class MythosRace:
    ...
    Usage
    -----
        race = MythosRace(scan_interval_seconds=3600)
    ...
    def __init__(
        ...
        *,
        ...
        self._findings: List[VulnerabilityCandidate] = []
    ...
    def scan_path(self, path: str) -> List[VulnerabilityCandidate]:
        """
    ...
    def scan_cfg(
        ...
        builder = SoftwareCategoryBuilder().from_cfg(nodes, edges)
    ...
    def _infer_hollow_gap_type(
        source_obj: SoftwareObject, target_obj: SoftwareObject
    ) -> CoherenceGapType:
    ...
    async def run(self, targets: List[str]):
        ...
        """
        print(f"[MythosRace] Starting continuous scan of {targets}")
        while True:
            all_candidates: List[VulnerabilityCandidate] = []
            ...
            if all_candidates:
                ...
                print("[MythosRace] No coherence gaps found — systems clean.")

            await asyncio.sleep(self.scan_interval)

    async def _emit(self, candidates: List[VulnerabilityCandidate]):
        """
        ...
        print(f"[MythosRace] {len(candidates)} gaps found — "
              f"{len(critical)} critical, {len(chainable)} chainable.")

        for c in candidates:
            print(
                f"  [{c.gap_type.value}] {c.vuln_class} "
    ...
    @property
    def all_findings(self) -> List[VulnerabilityCandidate]:
        return list(self._findings)

    @property
    def critical_findings(self) -> List[VulnerabilityCandidate]:
        return [f for f in self._findings if f.is_critical]

    @property
    def chainable_findings(self) -> List[VulnerabilityCandidate]:
        """
...
class FullDefensePipeline:
    ...
    def _build_category_from_builder(self, builder: SoftwareCategoryBuilder):
        """
        Convert SoftwareCategoryBuilder data into a Category for COG/OPTIMUS.
        """
    ...
    def _refine_with_optimus(
        self, vuln: VulnerabilityCandidate, optimus_engine
    ...
    def _decompose_chain(
        self, vuln: VulnerabilityCandidate
    ) -> VulnerabilityCandidate:
        """
        ...
        try:
            ...
            if 0 in priv_levels and 2 in priv_levels:
                trust_boundaries.append(TrustBoundary(
                    ...
                    policy_rules=["privilege_escalation_check"],
                ))
    ...
    def _predict_next_attacks(
        self, vulnerabilities: List[VulnerabilityCandidate],
    ) -> List[Dict]:
        """
    ...
    def run(
        self, builder: SoftwareCategoryBuilder, location: str = "",
    ) -> "DefenseReport":
        """
        ...
        Steps:
          ...
          3. Classify → VulnerabilityCandidate
...
class DefenseReport:
    ...
    @property
    def critical(self) -> List[VulnerabilityCandidate]:
        return [v for v in self.vulnerabilities if v.is_critical]

    @property
    def chainable(self) -> List[VulnerabilityCandidate]:
...
if __name__ == "__main__":
    ...
    print(f"  Vuln class:  {vuln.vuln_class}")
    ...
    for v in found:
        print(f"  {v.vuln_class} | severity={v.severity:.2f} | {v.remediation}")
