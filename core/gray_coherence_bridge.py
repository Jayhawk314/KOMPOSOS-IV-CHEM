...
on top:
    ...
    zfc_verifier.py      →  HOLLOW verdict = structurally plausible but
                            logically unsound = vulnerability candidate
...
memory-safety or logic vulnerability.
...
Minimal usage:
    ...
    from komposos.core.gray_coherence_bridge import MythosShield

    shield = MythosShield(oracle, zfc_bridge, learner)
...
class GapFinding:
    ...
    def is_chainable(self) -> bool:
        return self.vulnerability.is_chainable

    def __repr__(self) -> str:
        return (
            f"GapFinding({self.conjecture_source} → {self.conjecture_target} "
            f"| {self.vulnerability.vuln_class} "
...
class ShieldReport:
    """
    Full output of one MythosShield scan.
...
def _hollow_to_twocell_pair(
    ...
    The interchange failure between them is the vulnerability.
    """
...
class MythosShield:
    ...
    maps them to vulnerability classes, and returns patchable reports.
    ...
    def scan(
        ...
        by_gap: Dict[str, int] = {}
        for f in findings:
            by_class[f.vulnerability.vuln_class] = (
                by_class.get(f.vulnerability.vuln_class, 0) + 1
            )
            by_gap[f.vulnerability.gap_type.value] = (
                by_gap.get(f.vulnerability.gap_type.value, 0) + 1
            )
    ...
    def record_patch(
        ...
        Call this after a vulnerability is patched (or confirmed false positive).
    ...
    async def run_continuous(self, top_k: int = 100):
        ...
        """
        print(f"[MythosShield] Continuous scan started "
    ...
    def print_report(self, report: ShieldReport):
        ...
        for i, f in enumerate(report.findings[:20], 1):
            ...
            print(f"      vuln    : {f.vulnerability.vuln_class}")
            print(f"      gap     : {f.vulnerability.gap_type.value}")
            print(f"      severity: {f.combined_severity:.2f}")
            print(f"      zfc     : {f.zfc_verdict} (conf={f.zfc_confidence:.2f})")
            print(f"      mitre   : {f.vulnerability.mitre_id}")
            print(f"      fix     : {f.vulnerability.remediation}")
    ...
    def _get_conjectures(
        ...
        try:
            ...
            print(f"[MythosShield] ConjectureEngine error: {exc}")
            return []
    ...
    def _zfc_classify(
        ...
        try:
            ...
            print(f"[MythosShield] ZFC verify error: {exc}")
...
def build_shield(
    ...
    """
    Build a MythosShield with all available layers.
...
if __name__ == "__main__":
    print("MythosShield — Gray Coherence Bridge")
    print("=====================================")
    print()
