...
"""
Persistent Homology for Cyber Attack Detection

Topological Data Analysis (TDA) applied to network security:
- Compute persistent homology of network graphs
- Detect topological anomalies caused by cyber attacks
- Classify attack types via Betti number signatures

Mathematical Foundation:
- Build simplicial complexes from network connectivity
- Track birth/death of topological features across filtration
- H0 (connected components): network fragmentation / merging
- H1 (loops): communication cycles / botnet rings

Attack Signatures (Betti number changes):
- DDoS:  b0 increases (network fragmentation)
- Botnet: b1 increases (new communication loops)
- Worm:  b0 decreases (infection merges isolated components)
...
class TDAAttackDetector:
    """
    Detect cyber attacks using Topological Data Analysis.

    Compares the persistent homology of a baseline network graph
    against the current network state. Changes in Betti numbers
    indicate different attack signatures:

    - DDoS:  b0 increases (network fragmentation from overload)
    - Botnet: b1 increases (new command-and-control loops)
    - Worm:  b0 decreases (worm merges previously isolated segments)
    """

    def __init__(self, baseline_edges: List[Tuple[int, int]]):
        """
        Initialize with a baseline network graph.

        Args:
            baseline_edges: Edges of the normal network graph.
        """
    ...
    def detect_anomaly(self, current_edges: List[Tuple[int, int]]) -> Dict:
        ...
        """
        # Build current complex and compute persistence
        ...
        # Compare Betti numbers at t=2.0 (after all flag edges appear at 1.0)
        t = 2.0
        ...
        if b0_change > 0:
            # More components = fragmentation = DDoS
            attack_type = "DDoS"
            is_anomalous = True
        elif b1_change > 0:
            # More loops = new communication cycles = Botnet
            attack_type = "Botnet"
            is_anomalous = True
        elif b0_change < 0:
            # Fewer components = merging = Worm spreading
            attack_type = "Worm"
            is_anomalous = True
