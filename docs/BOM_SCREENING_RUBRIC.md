...
| "How accurate are the replacement scores?" | "Replacement scores are triage estimates. Covered pairwise interfaces use a deployed 98-row development/spent isotonic artifact (artifact OOS ECE 0.055); a broader post-squash study measured ECE 0.070, while Q11 blind ECE was 0.177. These are different cohorts, and calibration is not established per domain or for a full stack. A full-stack value is withheld if any requested contact is unscored. Pilot and qualify any replacement in the actual process." |
...
| "Why should I trust a computational tool?" | "The report exposes the evidence source, interface coverage, missing contacts, and calibration scope available for each result. Covered pairwise scores use a cohort-fitted probability mapping, but that is not laboratory validation and does not make an unscored full stack known. Treat the output as an auditable screening record and qualify the selected replacement experimentally." |
