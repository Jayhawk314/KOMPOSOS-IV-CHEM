...
| Crystal Dreamer | Searches candidate formulas by perturbation, interpolation, substitution, and stoichiometry variation, then scores with the forward predictor and physical gates. | The return-tail gate bug is fixed. A 2026-08-09 strict leave-one-anchor-out development run reproduced 7/9 self-consistent property-window recovery, 2/9 exact@25, and 6/9 near@25. Charge balance was assessed for 2,527/2,700 candidate decisions; 102 definite failures were vetoed and 71 remained unassessable. Receipt: `audit/crystal_recovery_report.json`. | An inverse-design lead generator whose quantitative ceiling is the same forward predictor used to define the targets. This is development/spent search-coverage evidence, not predictive accuracy, blind validation, or experimental recovery. |
