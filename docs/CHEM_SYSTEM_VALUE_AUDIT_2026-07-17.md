...
| Crystal Dreamer | Searches candidate formulas by perturbation, interpolation, substitution, and stoichiometry variation, then scores with the forward predictor and physical gates. | The return-tail gate bug is fixed. The strict leave-one-anchor-out development run reproduced 7/9 self-consistent property-window recovery, 2/9 exact@25, and 6/9 near@25. A matched search ablation found the four-strategy union tied simpler variants at 7/9; the 58-formula stoichiometry grid achieved 4/9 exact and 6/9 near with 100% gate coverage. Receipts: audit/crystal_recovery_report.json and audit/crystal_search_ablation_report.json. | A formula lead generator whose quantitative ceiling is the same forward predictor. The current 7/9 result does not establish incremental value from four-strategy orchestration; target-family templates explain much of the recovery. This is development/spent evidence, not predictive accuracy, blind validation, or experimental recovery. |
