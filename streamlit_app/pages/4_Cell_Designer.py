...
with col2:
    ...
    electrolyte = st.selectbox("Electrolyte", electrolyte_options, index=e_idx)

with col3:
    default_b = p.get("binder", BINDERS[0] if BINDERS else "")
    b_idx = BINDERS.index(default_b) if default_b in BINDERS else 0
    binder = st.selectbox("Binder (polymer)", BINDERS, index=b_idx)
...
with col4:
    ...
    collector = st.selectbox("Collector (metal)", COLLECTORS, index=c_idx)
